...
class valve:
    ...
    def __init__(self, valve_number,master):
        ...
        self.exp_info_logger.write_info("valve" + valve_number + "created")

    def open_valve(self):

        GPIO.output(self.solenoid_pin, GPIO.HIGH)
        self.exp_info_logger.write_info("valve" + self.valve_number + "opened")
        self.master.status['valve1'] = 1

    def close_valve(self):

        GPIO.output(self.solenoid_pin, GPIO.LOW)
        self.exp_info_logger.write_info("valve" + self.valve_number + "closed")
